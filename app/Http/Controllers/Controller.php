...
class Controller extends BaseController
{
    ...
    private function getJornadaActivaData($idTemporada, $jornadaActiva)
    {
        // Obtener información principal de torneo o liga
        $infoTorneoLiga = DB::select("SELECT 
        t.torneo_id,
        tor.tipo_torneo, 
        tor.betsapi,
        tor.nombre, 
        ...
        INNER JOIN categoria ce ON tor.categoria_id=ce.id WHERE t.id=$idTemporada");
        $torneoLiga = $infoTorneoLiga[0];
        ...
        WHERE p.temporada_id=$idTemporada AND p.jornada=$jornadaActiva
        ...
        return ['torneoLiga' => $torneoLiga, 'jornadaActivaData' => $jornadaActivaData];
    }
    ...
    public function indexCategorias(Request $request, $nombre, $id = null, $tabs = null)
    {
        ...
        if (!is_null($tabs)) {
            ...
            $datos = $this->getJornadaActivaData($id, $jornadaActiva);

            $torneoLiga = $datos['torneoLiga'];
            $jornadaActivaData = $datos['jornadaActivaData'];

            return response()->json(['data' => $jornadaActivaData, 'torneoLiga' => $torneoLiga, 'jornadaActiva' => $jornadaActiva]);
        }
    }
